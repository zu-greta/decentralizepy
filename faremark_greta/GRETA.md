...
| June 23 | [x] build basic federated learning framework <br> [1] test to make sure everything is correct <br> [] document and present <br> [] build the free-rider attacks <br> [] build the watermarking algorithm <br> [] test and validate everything is correct and matches the paper <br> [] document + double check with paper + present | [1] stage 1 tests: smoke test good + CIFAR-10 baseline (just FL) + ResNet-18/MNIST (just FL) <br> |

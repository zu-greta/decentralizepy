...
leg_iid(){ case "$1" in
  ...
  attacks) env DS=c100 bash "$RA" calibrate
...
leg_balanced(){ case "$1" in
  ...
  attacks) env DS=c100 BALANCED=1 VTAG=bal bash "$RA" calibrate
...
leg_noniid(){ case "$1" in
  ...
  attacks) env DS=c100 PART=niid bash "$RA" calibrate
...
leg_sin(){ case "$1" in
  ...
  attacks) env DS=c100 WMF=sin bash "$RA" calibrate
...
leg_bits20(){ case "$1" in
  ...
  attacks) env DS=c100 BITS=20 bash "$RA" calibrate
...
leg_classes(){ case "$1" in
  ...
  attacks) env DS=c100 VTAG=spread bash "$RA" calibrate
...
leg_capacity(){ case "$1" in
  ...
  attacks) env DS=c100 VTAG=nc200 NUM_CLIENTS="$CAP_NC" bash "$RA" calibrate
...
leg_capacity10(){ case "$1" in
  ...
  attacks) env DS=c10 NUM_CLIENTS="$CAP10_NC" bash "$RA" calibrate
...
case "$PHASE" in
  honest|attacks|plot)
    echo "=== run_everything: phase=$PHASE  legs=[$LEGS]  honest_seeds=[$HONEST_SEEDS] attack_seeds=[$ATTACK_SEEDS] BALANCED=$BALANCED ==="
    for leg in $LEGS; do
      echo "########## $leg / $PHASE ##########"
      "leg_$leg" "$PHASE"
    done
    ...
    case "$PHASE" in
      ...
      attacks) echo "next: wait for the attack jobs, then scp results to local and: RES=<local> ./run_everything.sh plot" ;;

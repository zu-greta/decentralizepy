#!/usr/bin/env bash
# =============================================================================
# run_now.sh -- builds jobs.tsv (the pool manifest) for a chosen set of groups.
# Called by runbook.sh manifest. Submits NOTHING (DRYRUN=1); submit_pool.sh runs it.
#
...
#   H  H5 crude previous-models free-rider (positive control, must be caught)
#   K  the submarine: K4 (block2, 3 seeds) + K5 (full scope, 3 seeds) [+K4b if K4B=1]
#   Z  NEW: no-watermark control (all-honest, lambda=0) for the trig_acc causation check
#   Y  NEW: J4 oracle-eta reproduction at classes 3,6 AND 1,7 (single seed)
#
# Most A/D/E/EA runs are done; their recipes are kept (commented) so any family can
# be regenerated by uncommenting its loop. The pool SKIPS a family/rep whose
# result.json already exists, so re-running is safe.
#
# WM_ETA_FIXED only draws the dashed detection line + fills the live FPR/recall
# columns; it does NOT decide separability (the frozen references live in plots.py).
# =============================================================================
set -uo pipefail
...
# ---------------------------------------------------------------------------
# GROUP A -- proven IID baseline (cifar100, 10 clients). Done; uncomment to regen.
# ---------------------------------------------------------------------------
if has A; then
...
# ---------------------------------------------------------------------------
# GROUP D -- +N reduced spectrum at the hard classes (3,6). Done; uncomment to regen.
# ---------------------------------------------------------------------------
if has D; then
...
# ---------------------------------------------------------------------------
# GROUP H -- H5 crude previous-models free-rider on c100 (positive control).
...
if has H; then
  for s in 0 1 2; do
    env ATTACK=previous_models NUM_FREE_RIDERS=2 FREE_RIDER_IDS=3,6 WM_ETA_FIXED=0.064 ROUNDS=50 \
        FAMILY="H5_prevmodel_c100" NOTE="H5 crude FR previous-models on c100 (positive control)" \
...
# ---------------------------------------------------------------------------
# GROUP Z -- NO-WATERMARK CONTROL (single seed).  [NEW]
#   All-honest run with the watermark EMBEDDING disabled (WM_LAMBDA=0) but the
#   verifier still active, so trig_acc IS logged. Purpose: confirm the ~0
#   trigger-class accuracy seen in A1 is caused by the watermark embedding (the
#   anti-dominance flattening, STORYLINE 6c) and NOT by class difficulty or a
#   logging artifact. With lambda=0 the model trains as plain FedAvg, so trig_acc
#   should stay HIGH. Plotted by runbook `plot` via honest_per_round.
#   ('Z' shares no letter with A C D E H K EA Y, so BATCH=Z fires ONLY this.)
# ---------------------------------------------------------------------------
if has Z; then
...
# ---------------------------------------------------------------------------
# GROUP Y -- J4 ORACLE-ETA REPRODUCTION (single seed).  [NEW]
...
#   instead of self-estimating it (TAP_ETA_SOURCE=oracle, fixed margin/warmup).
#   Reproduced at classes 3,6 (c36) and 1,7 (c17). Plotted by runbook `plot` via
#   tap_perfr + tap_effort.
#   ('Y' shares no letter with A C D E H K EA Z, so BATCH=Y fires ONLY this.)
# ---------------------------------------------------------------------------
if has Y; then
